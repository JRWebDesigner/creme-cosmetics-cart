        ...
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
              {items.length === 0 ? (
                <p className="mt-20 text-center text-sm text-muted-foreground">Tu bolso está vacío.</p>
              ) : (
                    ...
                    <li key={i.product.id} className="flex gap-4 rounded-2xl bg-secondary/40 p-3">
                      ...
                      <div className="flex flex-1 flex-col justify-between">
                        <div className="flex items-start justify-between">
                          ...
                          <button onClick={() => cart.remove(i.product.id)} className="text-xs text-muted-foreground hover:text-foreground">Quitar</button>
                        </div>
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2 rounded-full border border-border bg-background px-2 py-1">
                            <button onClick={() => cart.setQty(i.product.id, i.quantity - 1)} aria-label="Menos"><Minus className="h-3 w-3" /></button>
                            <span className="w-6 text-center text-sm">{i.quantity}</span>
                            <button onClick={() => cart.setQty(i.product.id, i.quantity + 1)} aria-label="Más"><Plus className="h-3 w-3" /></button>
                          </div>
                          <p className="font-display">${i.product.price * i.quantity}</p>
                        </div>
                      </div>
                    </li>
                  ))}
            ...
            <div className="border-t border-border px-6 py-5">
              ...
              <p className="mt-2 text-center text-xs text-muted-foreground">Envío gratis en pedidos +$50</p>
            </div>
          </motion.aside>
        </>
      )}
